function createGrid(numOfCellsInEachRow = 10) {
  ...
  for (let i = 0; i < numOfCellsInEachRow; i++) {
    for (let j = 0; j < numOfCellsInEachRow; j++) {
      ...
      let sizeOfCell = 600 / numOfCellsInEachRow;
      size = sizeOfCell + "px";
...
function makeNewGrid() {
  let numOfCellsInEachRow = prompt(
    "Enter number of cells per row and column for new grid[1-100]:"
  );
  if (numOfCellsInEachRow <= 100) {
    ...
    const numOfCellsInEachRow = confirm(
      "Enter a size less than or equal to 100."
    );
    makeNewGrid();
  }
}
